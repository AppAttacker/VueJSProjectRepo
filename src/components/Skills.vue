<template>
	<div class="hello">
		<h2>
			<GreetingPane v-bind:name="$route.params.name"></GreetingPane> {{ msg }}
		</h2>
			...
			<b-row class="text-left">
				<b-col>
					<b-button
						variant="info"
						name="btnMore"
						v-on:click="displayMoreInfo"
						>{{ !this.isActive ? 'More Info..' : 'Less..' }}</b-button
					>
					<div v-if="isActive">
						<form @submit.prevent="addSkill">
							...
							<div class="error" v-if="!$v.skill.minLength">
								Skill must have at least 4 letters.
							</div>
							<div class="error" v-if="!$v.skill.required">
								Field is required
							</div>
							<ol>
...
export default {
	...
	data() {
		return {
			...
			skills: [{ skill: 'Java' }, { skill: 'Vue.JS' }, { skill: 'ExtJS' }]
		};
	},
	mounted() {
		axios({
				...
				return this.list.filter(employee => {
					return (
						employee.employee_name
							.toLowerCase()
							.indexOf(this.filterVal.toLowerCase()) > 0
					);
				});
			} else {
				return this.list;
